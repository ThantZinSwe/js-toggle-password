import { applyStyles, createSvgElement } from "./style.js";
...
function togglePassword(
  ...
  customSvg = {
    ...
    invisible: `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
  ...
  applyStyles(svg, position);

  passwordFields.forEach((passwordField) => {
    const parentElement = passwordField.parentElement;
    parentElement.classList.add("tp-blk");

    const toggleSvg = document.createElement("span");
    toggleSvg.classList.add("tp-position");

    const visibleSvg = createSvgElement(customSvg.visible);
    const inVisibleSvg = createSvgElement(customSvg.invisible);
    ...
    toggleSvg.appendChild(visibleSvg.cloneNode(true));
